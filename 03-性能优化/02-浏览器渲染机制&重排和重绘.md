...
![](https://img.smyhvae.com/20210118-2005.png)
...
![](https://img.smyhvae.com/20210118-1950.jpg)
...
（1） reflow是问“怎么避免”，repaint是问“怎么减少”。Repaint是无法避免的，否则就成了静态页面了。

（2）重排的影响更大。重绘不一定重排，但重排一定会导致重绘。所以要尽量避免重排。
...
我们还需要通过 `Layout` 布局阶段，来计算它们在设备视口(viewport)内的确切位置和大小，计算这些值的过程称为`回流 flow`、`布局layout` 或`重排（Reflow）`。

**触发：**

DOM元素的**大小**和**位置**发生变化的时候，会触发布局 layout。
...
**定义**：当盒子的位置、大小以及其他属性，例如颜色、字体大小等都确定下来后，浏览器便把这些元素都按照各自的特性绘制一遍，于是页面的内容出现了，这个过程也称之为 Repaint（重绘制）。

说白了，页面要呈现的内容，统统画在屏幕上，这就叫 Repaint。其实，就是判断当视觉上是否发生变化（无论这个变化是通过DOM改动还是CSS改动）。只要页面显示的内容不一样了，肯定要 Repaint。
...
- [如何减少浏览器repaint和reflow ?](http://blog.csdn.net/liaozhongping/article/details/47057889)
