...
（4）注册回调事件。 注册onreadystatechange事件，状态改变时就会调用。
...
![](https://img.smyhvae.com/20180307_1443.png)

### 事件的触发顺序

![](https://img.smyhvae.com/20180307_1445.png)
...
（2）ajax请求时，浏览器要求前端和服务端必须同源。

### 同源策略的限制

同源策略的限制：这个源的文档没有权利去操作另一个源的文档。这个限制体现在：（要记住）

（1）不能发送Ajax请求。我们要注意，Ajax只适合**同源**的通信。

（2）Cookie、LocalStorage和IndexDB无法获取。
...
<script scr = 跨域的js地址></script>
```

后面两个很好理解，我们经常会使用一些第三方的CDN资源的地址。比如 bootstrap库等。
...
（1）所有的跨域，都必须服务端允许，并做配合。

（2）未在 server端允许就跨域了，说明网站有漏洞，有安全风险。

服务端 的 http header 需要做如下配置：

```js
response.setHeader("Access-Contol-Allow-Origin", "https://localhost:8081"); // 第二个参数一般填跨域的具体域名，不建议直接写 "*"
response.setHeader("Access-Contol-Allow-Headers", "X-Requested-With");
response.setHeader("Access-Contol-Allow-Methods", "PUT, POST, GET, DELETE, OPTIONS");

response.setHeader("Access-Contol-Allow-Credentials", "true"); // 接收跨域的 cookie
```
...
```html
    <script src="http://www.qianguyihao.com/?data=name&callback=myjsonp"></script>
```
